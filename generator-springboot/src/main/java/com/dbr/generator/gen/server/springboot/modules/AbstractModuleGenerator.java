...
public abstract class AbstractModuleGenerator {
    ...
    protected void generate(List<ModulItem> items) throws Exception {
        for (ModulItem item : items) {
            ...
            Template t = velocityEngine
                    .getTemplate(String.format("modules/%s", item.getTemplateName()));
    ...
    protected void copyResources(List<ResourceItem> items) throws IOException {
        for (ResourceItem item : items) {
            ...
            File destinationFile = new File(this.getClass().getResource("/").getPath() + "../../src/main/resources/" + item.getResourcePath());
    ...
    public void writeDown(ModulItem item, String content) throws Exception {
        String path = this.getClass().getResource("/").getPath() + "../../src/main/java/" + ResourceUtil.getPackageNameAsPath(this.packageName + item.getPackageSuffix());
